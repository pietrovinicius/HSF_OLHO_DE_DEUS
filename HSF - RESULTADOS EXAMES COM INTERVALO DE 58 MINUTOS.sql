...
SELECT DISTINCT
    '*'||'Prescrição: ' || PM.NR_PRESCRICAO || '*'                          AS NR_PRESCRICAO,
    'Paciente: ' || SUBSTR(obter_nome_pf(AP.CD_PESSOA_FISICA), 1, 1) || 
    ...
    --'ATEND: ' || AP.NR_ATENDIMENTO                                AS NR_ATENDIMENTO,
    'Ex: ' || upper(EL.NM_EXAME)                                     AS NM_EXAME,
    'Resultado: ' ||ELABI.QT_RESULTADO                                  AS RESULTADO,
    --'LIBERADOR POR: ' || ELABI.NM_USUARIO_LIBERACAO                     AS USUARIO_LIBERACAO,    
    'Col: ' || TO_CHAR(ELABI.DT_COLETA,'hh24:mi')                  AS DT_COLETA,
    'Dig: ' || TO_CHAR(ELABI.DT_DIGITACAO,'hh24:mi')               AS DT_DIGITACAO,
    'Apr: ' || nvl(TO_CHAR(ELABI.DT_APROVACAO,'hh24:mi'), ' - ')   AS DT_APROVACAO
    --'IMPRESSO: ' ||TO_CHAR(ELABI.DT_IMPRESSAO,'hh24:mi')                AS DT_IMPRESSAO
...
AND ELABI.QT_RESULTADO > 0
--AND ELABI.DT_APROVACAO IS NOT NULL
AND ELABI.DT_DIGITACAO >= SYSDATE - INTERVAL '59' MINUTE -- Pega exames digitados nos últimos 5 mi
--AND ELABI.DT_DIGITACAO >= SYSDATE - INTERVAL '7' HOUR -- Pega exames digitados nas últimas 24 horas
--AND UPPER(EL.NM_EXAME) LIKE UPPER('%HEMOGRAMA%')
AND (
    CASE    
        ...
        -- Regra específica e precisa para Cálcio Iônico
        WHEN UPPER(EL.NM_EXAME) = 'CÁLCIO IÔNICO ' AND (ELABI.QT_RESULTADO > 6.6 OR ELABI.QT_RESULTADO < 3.1) THEN 'CRÍTICO'
        WHEN UPPER(EL.NM_EXAME) = 'CÁLCIO IÔNICO' AND (ELABI.QT_RESULTADO > 6.6 OR ELABI.QT_RESULTADO < 3.1) THEN 'CRÍTICO'

        -- Regra para Cálcio Total/Sérico, garantindo que não se aplique ao iônico
        WHEN UPPER(EL.NM_EXAME) = UPPER('Cálcio ') AND UPPER(EL.NM_EXAME) <> 'CÁLCIO IÔNICO' AND (ELABI.QT_RESULTADO > 14 OR ELABI.QT_RESULTADO < 6.3) THEN 'CRÍTICO'
        WHEN UPPER(EL.NM_EXAME) = UPPER('Cálcio') AND UPPER(EL.NM_EXAME) <> 'CÁLCIO IÔNICO' AND (ELABI.QT_RESULTADO > 14 OR ELABI.QT_RESULTADO < 6.3) THEN 'CRÍTICO'
        ...
        WHEN UPPER(EL.NM_EXAME) LIKE UPPER('TAP%PROTROMBINA%') AND ELABI.QT_RESULTADO > 6 THEN 'CRÍTICO'
...
--    ELABI.DT_APROVACAO,
--    ELABI.DT_IMPRESSAO,
--    ELABI.NM_USUARIO_LIBERACAO
--ORDER BY EL.NM_EXAME, OBTER_NOME_PACIENTE(AP.NR_ATENDIMENTO) , ELABI.DT_DIGITACAO , ELABI.DT_IMPRESSAO                            
--ORDER BY ELABI.DT_DIGITACAO , OBTER_NOME_PACIENTE(AP.NR_ATENDIMENTO), EL.NM_EXAME
--ORDER BY 7,2,4
order by 6                    